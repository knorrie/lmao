...
if (isset($_POST['selectdc']) and $apuser->hasMailRelay($_POST['selectdc'])) {
...
# voor alle acties mbt mailrelay moet er een domein zijn waarbinnen we
...
switch ($action) {
	...
	case 'none':
		...
		# gebruiker wel rechten heeft om hier iets mee te doen...
			...
			if (!$apuser->hasMailRelay($dc)) $error = 1;
		}
		break;
	case 'newcn':
	case 'deletecn':
...
if ($error == 0) switch ($action) {
	case 'newcn':
		...
		# proberen de nieuwe relay toe te voegen
		$error = ($mailrelay->addRelay($cn)) ? 0 : 2;
		# als dat lukt gaan we naar het relay-edit schermpje toe meteen
		if ($error == 0) {
	...
	case 'deletecn':
		...
		# proberen de nieuwe relay toe te voegen
		$error = ($mailrelay->removeRelay($cn)) ? 0 : 2;
		# als dat lukt gaan we terug naar de lijst
		if ($error == 0) {
			header(sprintf("Location: %smailrelay.php?dc=%s", AP_URL, $dc));
			exit;
		}
		break;
}
...
if ($error == 0  or $error == 2) {
	require_once('class.mailrelaycontent.php');
	$content = new MailRelayContent($apuser, $mailrelay);
} else {
	# geen rechten
